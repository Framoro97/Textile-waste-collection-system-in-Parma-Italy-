...
class Truck(sp.Resource):
    def __init__(self, env, the_map:DMatrix, capacity:float, threshold: float, velocity:float, 
                     ...
                     t_unload: Callable[float, float] = lambda kg: 300, calendar:Optional[Calendar] = None):
        ...
        if len(self.f_route)!=0: self._optimize()
        # statistiche 
    ...
    #ipotesi routing sballato, aggiorna la lista
    def _optimize(self):
        s,e=self.f_route[0],self.f_route[-1]
        mid=self.f_route[1:-1]
        self.f_route=[s]
        while mid:
            cord=min(mid, key=lambda p: abs(p[0]-s[0])+abs(p[1]-s[1]))
            self.f_route.append(cord)
            mid.remove(cord)
            s=cord
        self.f_route.append(e)
    ...
    def _sub_route(self, route:list[Nd], T:Trip):
        ...
        while True:
            ...
            if self.is_full or len(route) == 1: # deve andare all'output o perchè pieno, o perchè ha finito il tour
                ...
                continue # il truck prosegue il tour
